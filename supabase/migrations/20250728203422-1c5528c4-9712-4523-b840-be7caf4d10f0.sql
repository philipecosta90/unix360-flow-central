...
-- 3. Atualizar políticas que devem ser só para super admin
DROP POLICY IF EXISTS "admin_empresa_stats_select" ON public.admin_empresa_stats;
CREATE POLICY "admin_empresa_stats_select_super_admin_only"
ON public.admin_empresa_stats
FOR SELECT
TO authenticated
USING (public.is_super_admin());

-- 4. Política para feedback - só super admin pode ver todos os feedbacks
...
-- 5. Políticas para subscriptions - empresas podem ver suas próprias, super admin vê todas
...
-- 6. Políticas para empresas - super admin vê todas, outros veem só a própria
...
-- 7. Políticas para invoices - super admin vê todas
...
-- 8. Atualizar função para impedir auto-promoção para admin
...
-- 9. Criar trigger para prevenir escalação de privilégios
