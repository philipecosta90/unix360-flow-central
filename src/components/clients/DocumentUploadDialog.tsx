  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: uploadData,
        error: uploadError
      } = await supabase.storage.from('client-documents').upload(filePath, selectedFile);
      if (uploadError) {
      ...
      const {
        error
      } = await (supabase as any).from('cliente_documentos').insert([{
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input id="nome" value={fileName} onChange={e => setFileName(e.target.value)} placeholder="Nome do documento" required />
          </div>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
              ...
              <div className="mt-4">
                <label htmlFor="arquivo" className="cursor-pointer">
                  <span className="mt-2 block text-sm font-medium text-slate-50">
                    {selectedFile ? selectedFile.name : "Clique para selecionar um arquivo"}
                  </span>
                  <input id="arquivo" type="file" className="sr-only" onChange={handleFileChange} accept=".pdf,.doc,.docx,.txt,.jpg,.jpeg,.png" />
                </label>
              </div>
            </div>
          </div>
