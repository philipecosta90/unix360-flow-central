...
import { User, Mail, Phone, Calendar, MessageSquare, CheckCircle, Clock, AlertTriangle } from "lucide-react";
      ...
      <DialogContent className="w-[95vw] max-w-4xl max-h-[90vh] overflow-y-auto mx-2 sm:mx-4">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-1 gap-3 sm:gap-4">
                {cliente.email && <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                    <Mail className="h-4 w-4 text-gray-500 flex-shrink-0" />
                    <span className="text-sm break-all text-slate-950">{cliente.email}</span>
                  </div>}
                {cliente.telefone && <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                    <Phone className="h-4 w-4 text-gray-500 flex-shrink-0" />
                    <span className="text-sm text-gray-950">{cliente.telefone}</span>
                  </div>}
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div className="bg-[#43B26D] h-3 rounded-full transition-all duration-300" style={{
                  width: `${percentualOnboarding}%`
                }} />
                  </div>
                  <div className="space-y-3">
                    {onboarding.map(step => <div key={step.id} className="flex items-start gap-3 p-2 bg-gray-50 rounded-lg">
                        {step.concluido ? <CheckCircle className="h-4 w-4 text-green-600 flex-shrink-0 mt-1" /> : <Clock className="h-4 w-4 text-gray-400 flex-shrink-0 mt-1" />}
          ...
          <Card>
            ...
            <CardContent>
              {interacoes.length > 0 ? <div className="space-y-3">
                  {interacoes.slice(0, 5).map(interacao => <div key={interacao.id} className="flex items-start gap-3 p-3 border rounded-lg bg-white hover:bg-gray-50 transition-colors">
                      <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                        <MessageSquare className="h-4 w-4 text-blue-600" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-1 xs:gap-2">
                          <h4 className="font-medium text-sm leading-tight truncate text-slate-950">{interacao.titulo}</h4>
                          <span className="text-xs text-gray-500 flex-shrink-0">
                            {format(new Date(interacao.data_interacao), "dd/MM/yyyy", {
                        locale: ptBR
                      })}
                          </span>
                        </div>
                        <p className="text-sm text-gray-600 mt-1">{interacao.tipo}</p>
                        {interacao.descricao && <p className="text-sm text-gray-700 mt-2 leading-relaxed break-words">{interacao.descricao}</p>}
                      </div>
                    </div>)}
